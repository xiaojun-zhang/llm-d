...
RELEASE_NAME="${RELEASE_NAME:-llmd-kimivla3b-2e1pd}"
...
echo "B60 encoder cards: sc09intel02-b60 XPU 2 PCI 0000:54:00.0, XPU 3 PCI 0000:58:00.0"
...
kubectl rollout status deployment/llmd-kimivla3b-2e1pd-sglang-encode-0 \
  -n "${NAMESPACE}" --timeout=45m
kubectl rollout status deployment/llmd-kimivla3b-2e1pd-sglang-encode-1 \
  -n "${NAMESPACE}" --timeout=45m
kubectl rollout status deployment/llmd-kimivla3b-2e1pd-sglang-decode \
  -n "${NAMESPACE}" --timeout=45m
